...
void DetectionLossLayer<Dtype>::Forward_cpu(
  ...
  Dtype recall_value(0.0);
  int obj_count = 0;
  int recall_flag = 0;
  int locations = pow(side_, 2);
  int recall_count = 0;
  Dtype best_classprob(-0.1);
  Dtype best_scoreprob(-0.1);
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    ...
    for (int j = 0; j < locations; ++j) {
	  ...
	  for (int k = 0; k < num_object_; ++k) {
		  ...
		  /*LOG(INFO) << "tx: " << true_box[0];
		  LOG(INFO) << "ty: " << true_box[1];
		  LOG(INFO) << "tw: " << true_box[2];
		  LOG(INFO) << "th: " << true_box[3];*/
		  Dtype iou = Calc_iou(box, true_box);
		  if ((recall_flag == 0) && (iou > 0.5))
		  {
			  recall_flag = 1;
	  ...
	  // rescore
	  diff[p_index] = object_scale_ * (input_data[p_index] - 1.0);
  ...
  recall_value = recall_count / (Dtype)obj_count;

  loss = class_loss + coord_loss + area_loss + obj_loss + noobj_loss;
  Dtype aobj_count = ((Dtype)obj_count) / bottom[0]->num();
  top[0]->mutable_cpu_data()[0] = loss;
  /*LOG(INFO) << "coord_scale_: " << coord_scale_ << endl;
  LOG(INFO) << "noobject_scale_: " << noobject_scale_ << endl;
  LOG(INFO) << "class_scale_: " << class_scale_ << endl;
  LOG(INFO) << "object_scale_: " << object_scale_ << endl;*/
  ...
  LOG(INFO) << "avg_iou: " << avg_iou << " avg_obj: " << avg_obj << " avg_no_obj: "
	  << avg_no_obj << " avg_cls: " << avg_cls << " avg_pos_cls: " << avg_pos_cls << " avg_recall_value:" << recall_value << " true_obj_nums:" << aobj_count;
}
