...
void BoxDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const DetectionVersionParameter version_param = this->layer_param_.detection_version_param();
  const int batch_size = param.batch_size();
  // Read a data point, and use it to initialize the top blob.
  Datum& datum = *(reader_.full().peek());
  yolo_version_flag_ = version_param.yoloversion();
  ...
  if (this->output_labels_) {
    ...
    for (int i = 0; i < sides_.size(); ++i) {
	  ...
	  int label_size = 0;
	  if (yolo_version_flag_ == 1)
	  {
		  label_size = sides_[i] * sides_[i] * (1 + 1 + 1 + 4);
	  }
	  else
	  {
		  label_size = 30*(1+4);//(maxboxes=30)*(4+1)
	  }
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (this->output_labels_) {
      ...
      for (int i = 0; i < sides_.size(); ++i) {
		...
		transform_label(count, top_label[i] + label_offset, box_labels, sides_[i], yolo_version_flag_);
      }
    } else {
...
void BoxDataLayer<Dtype>::transform_label(int count, Dtype* top_label,
    const vector<BoxLabel>& box_labels, int side, int ver) {
	if (1 == ver)
	{
...
#ifdef FILE_DEBUG
		FILE *fp = 0;
		fopen_s(&fp, "D:/Code_local/caffe/caffe_yolo/debug_result/GroudTruth_info.txt", "w+");
#endif
...
#ifdef FILE_DEBUG
			fprintf(fp, "%f ", class_label);
#endif
			for (int j = 0; j < 4; ++j) {
				top_label[cor_index + j] = box_labels[i].box_[j];
#ifdef FILE_DEBUG
				fprintf(fp, "%f ", box_labels[i].box_[j]);
#endif
			}
#ifdef FILE_DEBUG
			fprintf(fp, "\n");
