  ...
  if (softmax_tree != ""){
    ...
    *avg_cat += pred;
    ...
    for (int n = 0; n < classes; ++n){
      ...
      if (n == class_label){
		  tempsum = tempsum + input_data[index + n];
...
void RegionLossLayer<Dtype>::Forward_cpu(
  ...
  for (int b = 0; b < swap.num(); ++b){
	...
	//first Cyclic traversal,clac the noobj loss
    for (int j = 0; j < side_; ++j)
      for (int i = 0; i < side_; ++i)
        for (int n = 0; n < num_; ++n){
          int index = b * swap.channels() * swap.height() * swap.width() + (j * side_ + i) * swap.height() * swap.width() + n * swap.width();
          ...
          diff[index + 4] = (-1.0) * noobject_scale_ * (0.0 - swap_data[index + 4]) * (swap_data[index + 4]) * (1.0 - swap_data[index + 4]);//simmilar as weightdecay
          ...
          if (iter < 12800 / bottom[0]->num()){
			//during the early iteration, noobj_location loss should be calced
            vector<Dtype> truth;
            truth.clear();
    ...
    //LOG(INFO) << "obj ok"; 
	//second cyclic travelsal, calc the obj_loss,region_loss and class_loss
    for (int t = 0; t < 30; ++t){
      vector<Dtype> truth;
      truth.clear();
      ...
      diff[best_index + 4] = (-1.0) * object_scale_ * (1.0 - swap_data[best_index + 4]) * (swap_data[best_index + 4] * (1.0 - swap_data[best_index + 4]));
	  LOG(INFO) << "avg_obj_value: " << swap_data[best_index + 4];
      //if (rescore)
  ...
  //if (!(iter % 100))
  {
	  LOG(INFO) << "avg_noobj: " << avg_anyobj / ((Dtype)(side_*side_*num_*bottom[0]->num())) << " avg_obj: " << avg_obj / fcount << " avg_iou: " << avg_iou / fcount << " avg_cat: " << avg_cat / fclass_count << " recall: " << recall / fcount << " class_count: " << class_count << " effective_count: " << count;
  }
}
